...
// Search Options Change Location Dropdowns
    ...
    $("#searchNowrbtn").click(function () {
        var id = $("#majorcategory").val();
        if (id != -1) {
            var subid = $("#subcategory").val();
            if (subid == -1) {
                $("#subcategory").empty();
                $("#subcategory").append('<option value="'+id+'" selected>ALL</option>');
                //$("#subcategory").val(id)
            }
        }
        $("#SearchForm").submit();
   ...
   /* $("#searchNowbtn").click(function () {
        updateProviderResults();
    });

    function updateProviderResults() {
        var serviceCategoryID = $("#majorcategory").val();
        if(serviceCategoryID != -1){
            serviceCategoryID = $("#subcategory").val();
        }
        var mainSearchInput = $("#mainSearchInput").val();
